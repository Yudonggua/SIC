...
with open('loc.txt', 'r') as f:
    for line in f.readlines():
        instruction = line.split()
        if (len(instruction) != 4) or instruction[1] == 'FIFTH':
            continue
        else:
        ...
        with open('objcode.txt', 'a') as f_objcode:
            ...
            if line.startswith('.'):
                ...
                if ',X' in instruction[3]:
                    ...
                    print(
                        line + '\t' + "{:02X}{:X}".format(instruction_set[instruction[2]], sym_tmp))
                    f_objcode.write(
                        line + '\t' + "{:02X}{:X}".format(instruction_set[instruction[2]], sym_tmp) + '\n')
                else:
                    print(
                        line + '\t' + "{:02X}".format(instruction_set[instruction[2]]) + sym_table[instruction[3]])
                    f_objcode.write(line + '\t' + "{:02X}".format(
                        instruction_set[instruction[2]]) + sym_table[instruction[3]] + '\n')
                ...
                if ',X' in instruction[2]:
                    ...
                    sym_tmp = int(sym_table[tmp], 16) + 0x8000
                    print(
                        line + '\t' + "{:02X}{:X}".format(instruction_set[instruction[1]], sym_tmp))
                    f_objcode.write(
                        line + '\t' + "{:02X}{:X}".format(instruction_set[instruction[1]], sym_tmp) + '\n')
                else:
                    print(
                        line + '\t' + "{:02X}".format(instruction_set[instruction[1]]) + sym_table[instruction[2]])
                    f_objcode.write(line + '\t' + "{:02X}".format(
                        instruction_set[instruction[1]]) + sym_table[instruction[2]] + '\n')

            elif (len(instruction) == 2) and (instruction[1] == 'RSUB'):
                print(line + '\t\t' +
                      "{:02X}".format(instruction_set[instruction[1]]) + '0000')
                f_objcode.write(
                    line + '\t\t' + "{:02X}".format(instruction_set[instruction[1]]) + '0000' + '\n')
                ...
                if (instruction[2] == 'RESW') or (instruction[2] == 'RESB'):
                    ...
                    f_objcode.write(
                        line + '\t' + "{:06X}".format(int(instruction[3])) + '\n')
...
with open('objcode.txt', 'r') as f:
    ...
    for line in f.readlines():
        with open('object_program.txt', 'a') as f_objp:
            ...
            if 'START' in instruction:
            ...
            elif ('FIFTH' in instruction) and ('END' not in instruction):
                start = int(instruction[0], 16)
                objcode += instruction[len(instruction)-1]
                plength = int(loc_list[len(loc_list)-1],
                              16) - int(loc_list[0], 16)
                print('H' + pname + "{:06X}{:06X}".format(start, plength))
                f_objp.write(
                    'H' + pname + "{:06X}{:06X}".format(start, plength) + '\n')
                continue
                ...
                print('T' + "{:06X}{:02X}".format(start,
                      objcode_length) + objcode)
                f_objp.write(
                    'T' + "{:06X}{:02X}".format(start, objcode_length) + objcode + '\n')
                start = int(instruction[0], 16)
                objcode = ''
                objcode += instruction[len(instruction)-1]
                obj_cut = False
                ...
                f_objp.write(
                    'E' + "{:06X}".format(int(loc_list[0], 16)) + '\n')
